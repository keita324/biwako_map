...
有料観覧席エリア図「13」の範囲（打出の森〜びわ湖ホール〜ピアザ淡海の先の
河川まで）をブロック分けし、各ブロックの混雑度（1〜5）を入力すると
グラデーションのヒートマップとして色分けされた画像（PNG）を生成できる
ツールです。GitHub Pages だけで動作します（サーバー・データベース不要）。
...
```js
{ id: "A1", area: "打出の森 湖側", x: 115, y: 282, w: 110, h: 95, rot: -30 },
```
